...
// ─── Retounen lis nimewo sesyon ki konekte aktyèlman (itilize pa .sessions) ─
export function listSessionNumbers() {
    return Object.keys(sessions);
}

// ─── Dekonekte + retire yon sesyon (itilize pa .depair / .unpair) ──────────
export async function logoutSession(number) {
    ...
    if (sock) {
        try {
            ...
            console.error(`❌ Failed to logout session for ${number}:`, err?.message || err);
        }
    }
