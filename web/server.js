...
import { URL } from 'url';
import { startSession, logoutSession } from '../utils/connector.js';
...
<button class="sound" id="logoutBtn">${icon('x')}<span>Disconnect this session (de-pair)</span></button>
...
document.getElementById('logoutBtn').addEventListener('click',async()=>{
  if(!confirm('Disconnect this WhatsApp session? You will need to pair again.'))return;
  try{
    const r=await fetch('/api/logout',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({number:numberEl.value?cleanNumber(numberEl.value):undefined})});
    const d=await r.json();
    setStatus(d.message||'Session disconnected.','good');
    codeBox.classList.remove('show');
  }catch(e){
    setStatus('Could not disconnect. Try again.','bad');
  }
});
    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === 'POST' && url.pathname === '/api/logout') {
                ...
                const number = String(body.number || pairing?.number || '').replace(/\D/g, '');
                if (!number) {
                    return json(res, 400, { message: 'No active session number to disconnect.' });
                }
                await logoutSession(number);
                pairing = null;
                return json(res, 200, { message: 'Session disconnected. You can pair again anytime.' });
            }
